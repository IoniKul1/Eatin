...
class _Table extends DbTable
{
	...
	public function __construct()
	{
		...
		$this->TableVar = '_Table';
		$this->TableName = 'Table';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[Table]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new DbField('_Table', 'Table', 'x_ID', 'ID', '[ID]', 'CAST([ID] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Restaurant
		$this->ID_Restaurant = new DbField('_Table', 'Table', 'x_ID_Restaurant', 'ID_Restaurant', '[ID_Restaurant]', 'CAST([ID_Restaurant] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Restaurant]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// QRCode
		$this->QRCode = new DbField('_Table', 'Table', 'x_QRCode', 'QRCode', '[QRCode]', '[QRCode]', 202, 500, -1, FALSE, '[QRCode]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->QRCode->Required = TRUE; // Required field
		$this->QRCode->Sortable = TRUE; // Allow sort
		$this->fields['QRCode'] = &$this->QRCode;

		// Numero
		$this->Numero = new DbField('_Table', 'Table', 'x_Numero', 'Numero', '[Numero]', 'CAST([Numero] AS NVARCHAR)', 3, 4, -1, FALSE, '[Numero]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[Table]";
	}
	...
	{
		global $Language;
		if ($pageName == "_Tableview.php")
			return $Language->phrase("View");
		elseif ($pageName == "_Tableedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "_Tableadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("_Tableview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("_Tableview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "_Tableadd.php?" . $this->getUrlParm($parm);
		else
			$url = "_Tableadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("_Tableedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("_Tableadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("_Tabledelete.php", $this->getUrlParm());
	}
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM [dbo].[Table]";
