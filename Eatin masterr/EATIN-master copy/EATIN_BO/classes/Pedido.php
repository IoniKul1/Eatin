...
class Pedido extends DbTable
{
	...
	public function __construct()
	{
		...
		// ID
		$this->ID = new DbField('Pedido', 'Pedido', 'x_ID', 'ID', '[ID]', 'CAST([ID] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Client
		$this->ID_Client = new DbField('Pedido', 'Pedido', 'x_ID_Client', 'ID_Client', '[ID_Client]', 'CAST([ID_Client] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Client]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->ID_Client->IsForeignKey = TRUE; // Foreign key field
		...
		// ID_Status
		$this->ID_Status = new DbField('Pedido', 'Pedido', 'x_ID_Status', 'ID_Status', '[ID_Status]', 'CAST([ID_Status] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Status]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Restaurant
		$this->ID_Restaurant = new DbField('Pedido', 'Pedido', 'x_ID_Restaurant', 'ID_Restaurant', '[ID_Restaurant]', 'CAST([ID_Restaurant] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Restaurant]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// DateCreation
		$this->DateCreation = new DbField('Pedido', 'Pedido', 'x_DateCreation', 'DateCreation', '[DateCreation]', CastDateFieldForLike("[DateCreation]", 1, "DB"), 135, 8, 1, FALSE, '[DateCreation]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateLastUpdate
		$this->DateLastUpdate = new DbField('Pedido', 'Pedido', 'x_DateLastUpdate', 'DateLastUpdate', '[DateLastUpdate]', CastDateFieldForLike("[DateLastUpdate]", 0, "DB"), 135, 8, 0, FALSE, '[DateLastUpdate]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Table
		$this->ID_Table = new DbField('Pedido', 'Pedido', 'x_ID_Table', 'ID_Table', '[ID_Table]', 'CAST([ID_Table] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Table]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->ID_Table->Nullable = FALSE; // NOT NULL field
		$this->ID_Table->Required = TRUE; // Required field
		$this->ID_Table->Sortable = TRUE; // Allow sort
		$this->ID_Table->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->ID_Table->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
		switch ($CurrentLanguage) {
			case "en":
				$this->ID_Table->Lookup = new Lookup('ID_Table', '_Table', FALSE, 'ID', ["Numero","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->ID_Table->Lookup = new Lookup('ID_Table', '_Table', FALSE, 'ID', ["Numero","","",""], [], [], [], [], [], [], '', '');
				break;
		}
		$this->ID_Table->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ID_Table'] = &$this->ID_Table;
	}
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "Client") {
			if ($this->ID_Client->getSessionValue() != "")
				$masterFilter .= "[ID]=" . QuotedValue($this->ID_Client->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "Client") {
			if ($this->ID_Client->getSessionValue() != "")
				$detailFilter .= "[ID_Client]=" . QuotedValue($this->ID_Client->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "ItemxPedido") {
			$detailUrl = $GLOBALS["ItemxPedido"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_ID=" . urlencode($this->ID->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "Pedidolist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[Pedido]";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->ID_Table->DbValue = $row['ID_Table'];
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "Pedidoedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "Pedidoadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("Pedidoview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("Pedidoview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "Pedidoadd.php?" . $this->getUrlParm($parm);
		else
			$url = "Pedidoadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("Pedidoedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("Pedidoedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("Pedidoadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("Pedidoadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("Pedidodelete.php", $this->getUrlParm());
	}
	...
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "Client" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			...
			$url .= "&fk_ID=" . urlencode($this->ID_Client->CurrentValue);
		}
		return $url;
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->ID_Table->setDbValue($rs->fields('ID_Table'));
	}
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->ID_Table);
				} else {
					$doc->exportCaption($this->ID);
					$doc->exportCaption($this->ID_Client);
					$doc->exportCaption($this->ID_Status);
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM [dbo].[Pedido]";
