...
/**
 * Table class for Checkin
 */
class Checkin extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'Checkin';
		$this->TableName = 'Checkin';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[Checkin]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new DbField('Checkin', 'Checkin', 'x_ID', 'ID', '[ID]', 'CAST([ID] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Client
		$this->ID_Client = new DbField('Checkin', 'Checkin', 'x_ID_Client', 'ID_Client', '[ID_Client]', 'CAST([ID_Client] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Client]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Restaurant
		$this->ID_Restaurant = new DbField('Checkin', 'Checkin', 'x_ID_Restaurant', 'ID_Restaurant', '[ID_Restaurant]', 'CAST([ID_Restaurant] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Restaurant]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateCreation
		$this->DateCreation = new DbField('Checkin', 'Checkin', 'x_DateCreation', 'DateCreation', '[DateCreation]', CastDateFieldForLike("[DateCreation]", 0, "DB"), 135, 8, 0, FALSE, '[DateCreation]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[Checkin]";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "Checkinedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "Checkinadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("Checkinview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("Checkinview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "Checkinadd.php?" . $this->getUrlParm($parm);
		else
			$url = "Checkinadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("Checkinedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("Checkinadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("Checkindelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		if ($this->ID_Client->getSessionValue() != "") {
			...
			$this->ID_Client->EditValue = $this->ID_Client->CurrentValue;
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM [dbo].[Checkin]";
