...
/**
 * Table class for ItemOptions
 */
class ItemOptions extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'ItemOptions';
		$this->TableName = 'ItemOptions';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[ItemOptions]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new DbField('ItemOptions', 'ItemOptions', 'x_ID', 'ID', '[ID]', 'CAST([ID] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Item
		$this->ID_Item = new DbField('ItemOptions', 'ItemOptions', 'x_ID_Item', 'ID_Item', '[ID_Item]', 'CAST([ID_Item] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Item]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Restaurant
		$this->ID_Restaurant = new DbField('ItemOptions', 'ItemOptions', 'x_ID_Restaurant', 'ID_Restaurant', '[ID_Restaurant]', 'CAST([ID_Restaurant] AS NVARCHAR)', 20, 8, -1, FALSE, '[ID_Restaurant]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[ItemOptions]";
	}
	...
	{
		global $Language;
		if ($pageName == "ItemOptionsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "ItemOptionsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "ItemOptionsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("ItemOptionsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("ItemOptionsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "ItemOptionsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "ItemOptionsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ItemOptionsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ItemOptionsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("ItemOptionsdelete.php", $this->getUrlParm());
	}
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM [dbo].[ItemOptions]";
